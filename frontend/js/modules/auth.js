...
function updatePasswordStrength(event) {
    const passwordInput = event?.target || document.querySelector("[data-auth-password]");
...
async function handleRegisterSubmit(form) {
    ...
    try {
        ...
        if (!token || !user) {
            throw new Error("Tạo tài khoản thành công nhưng chưa lấy được phiên đăng nhập.");
        }
...
function bindDemoFillButtons() {
    document.querySelectorAll("[data-fill-demo]").forEach((button) => {
        button.addEventListener("click", () => {
            ...
            if (emailInput) {
                emailInput.value = button.dataset.email || "hieu@example.com";
